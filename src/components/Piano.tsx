...
const PianoComponent: React.FC<PianoProps> = ({ maxWidth, goBack }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Tone.Transport.state === "started") setPlaybackTime(Tone.Transport.seconds);
    }, 100);
    ...
    <div style={containerStyle}>
      <style>{`
        ...
        .ReactPiano__Key--natural {
          ...
          z-index: 1;
          width: ${pianoDisplayWidth / 14}px !important; 
        }
        ...
        .ReactPiano__Key--accidental {
          ...
          width: ${pianoDisplayWidth / 22}px !important;
          margin-left: -${pianoDisplayWidth / 44}px !important;
          margin-right: -${pianoDisplayWidth / 44}px !important;
      ...
      <div style={pianoChassisStyle}>
        <div style={toolbarStyle}>
          <div style={{display: 'flex', flexDirection: 'column', gap: '8px'}}>
            <label style={{color: '#888', fontSize: '12px', fontWeight: '900', letterSpacing: '1px'}}>IMPORT MUSICXML</label>
            <input type="file" accept=".musicxml,.xml" onChange={handleFileLoad} style={{fontSize: '14px', color: '#ccc'}} />
          </div>
        ...
        <div style={pianoWrapperStyle}>
          {!isLoaded ? (
            <div style={{color: '#facc15', fontSize: '24px', fontWeight: 'bold'}}>LOADING HIGH-FIDELITY AUDIO...</div>
          ) : (
        ...
        <div style={{ padding: "0 40px 60px", textAlign: 'left' }}>
          <button onClick={goBack} style={backButtonStyle}>← EXIT TO HOME</button>
        </div>
      </div>
    </div>
  );
